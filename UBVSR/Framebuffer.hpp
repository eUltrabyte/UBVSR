...
class FrameBuffer
{
  public:
	inline explicit FrameBuffer(std::uint16_t t_width, std::uint16_t t_height) noexcept
		: m_width{t_width}, m_height{t_height}, m_color_buffer{m_width,
															   m_height}, // m_ms_color_buffer{m_width, m_height},
		  m_ms_depth_buffer{m_width, m_height},                           // m_ms_stencil_buffer{m_width, m_height},
		  m_ms_pixel_info_buffer{m_width, m_height}, total_threads{std::clamp(std::thread::hardware_concurrency(), 1U,
																			  1024U)},
		  thread_states(total_threads, ThreadState::IDLE)
	...
	static constexpr bool cull_test(const std::array<Vertex, 3> &t_vertices) noexcept
	{
	...
	static constexpr std::array<std::array<Vertex, 3>, 2> clip_with_one_wrong_vertex(
		const Vertex &t_wrong_vertex, const std::array<Vertex, 2> &t_correct_vertices) noexcept
	{
	...
	struct TriangleInfo
	{
		...
		//std::array<float, 3> true_distances;
	};
	...
	inline void prepare_triangle(const std::array<Vertex, 3> &t_vertices, const Texture &t_texture)
	{
		...
		std::uint32_t end_x =
			std::min<std::uint32_t>(std::max<float>({vertices[0].x, vertices[1].x, vertices[2].x}) + 1, ms_width);

		std::uint32_t start_y =
			std::max<float>(std::min<float>({vertices[0].y, vertices[1].y, vertices[2].y}) - 1.0F, 0.0F);
		std::uint32_t end_y =
			std::min<std::uint32_t>(std::max<float>({vertices[0].y, vertices[1].y, vertices[2].y}) + 1, ms_height);
		...
		const static auto aspect_ratio_squared = aspect_ratio * aspect_ratio;

		// const std::array<float, 3> x_values{ t_vertices[0].position.x * aspect_ratio, t_vertices[1].position.x *
		// aspect_ratio , t_vertices[2].position.x * aspect_ratio }; const std::array<float, 3> y_values{
		// t_vertices[0].position.y, t_vertices[1].position.y , t_vertices[2].position.y };

		prepared_triangles.push_back(TriangleInfo{t_vertices, &t_texture, ndc_vertices, inverted_w_values, start_x,
												  end_x, start_y / total_threads * total_threads, end_y, true_area_abc,
												  inv_true_area_abc, constant1C, constant2C, constant3C, constant1B,
												  constant2B, constant3B});
	}
  ...
  private:
	...
	inline void draw_prepared_triangles(unsigned t_thread_id)
	{
		...
		for (std::size_t i = 0; i < prepared_triangles_size; ++i)
		{
			...
			//const auto &distances = triangle.true_distances;
			...
			for (std::uint32_t y = start_y + t_thread_id; y < end_y; y = y + total_threads)
			{
				...
				for (std::uint32_t x = triangle_start_left_x; x <= triangle_end_right_x; ++x)
				{
					...
					const auto inverted_w_value = (inverted_w_values[0] * scales[0] + inverted_w_values[1] * scales[1] +
					...
					{
						const float x_value = (ndc_vertices[0].x * scales[0] + ndc_vertices[1].x * scales[1] +
											   ndc_vertices[2].x * scales[2]) *
											  w_value * aspect_ratio;

						const float y_value = (ndc_vertices[0].y * scales[0] + ndc_vertices[1].y * scales[1] +
											   ndc_vertices[2].y * scales[2]) *
											  w_value;

						distance = (w_value * w_value + x_value * x_value + y_value * y_value);
					}

					static const float fog_end_squared = fog_params.end * fog_params.end;

					if (fog_params.clip && distance >= fog_end_squared)
					{
						continue;
					}
					...
					{
						m_ms_pixel_info_buffer.at(x, y) =
							pixel_info{&t_texture, distance,
									   fvec2{((t_vertices[0].texture_uv.x * inverted_w_values[0]) * scales[0] +
	...
	[[nodiscard]] inline Pixel sample_pixel(std::uint32_t t_x, std::uint32_t t_y) const noexcept
	{
		...
		const auto true_distance = std::sqrt(pixel_info.true_distance);
		auto pixel = pixel_info.texture->sample(pixel_info.uv);
		if (true_distance > fog_params.start)
		{
			const static auto inverted_fog_disance = 1.0F / (fog_params.end - fog_params.start);
			const auto fraction =
				std::sqrt(std::clamp((true_distance - fog_params.start) * inverted_fog_disance, 0.0F, 1.0F)) *
				fog_params.destiny;
	...
	inline void sample_simple(unsigned t_thread_id) noexcept
	{
		for (std::uint32_t x = t_thread_id; x < m_width; x = x + total_threads)
		{
			...
			{
				m_color_buffer.at(x, y) = sample_pixel(x, y);
			}
		}
	}
			...
			for (std::uint32_t y = 0; y < m_height; ++y)
			{
				...
				for (std::uint32_t mx = 0; mx < m_multisample; ++mx)
				{
					...
					for (std::uint32_t my = 0; my < m_multisample; ++my)
					{
						...
						const auto pixel = sample_pixel(index.x, index.y);
				...
				m_color_buffer.at(x, y) = Pixel(std::accumulate(r_values.begin(), r_values.end(), 0U) / ms_times_ms,
